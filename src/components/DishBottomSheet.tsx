...
export default function DishBottomSheet({ dish, isOpen, quantity, onClose, onUpdateQuantity, lang = "FR" }: DishBottomSheetProps) {
    ...
    const renderBadgeItem = (b: BadgeObject | string) => {
        if (typeof b === 'string') {
            return (
                <div key={b} className="flex items-center gap-2 bg-text-primary/10 border border-text-primary/10 px-3 py-1.5 rounded-full">
        ...
        return (
            <div key={b.key || b.label} className="flex items-center gap-2 bg-text-primary/5 border border-border px-3 py-1.5 rounded-full">
                ...
                <span className="text-[10px] font-bold tracking-widest text-text-primary uppercase">
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-end justify-center">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-md bg-surface rounded-t-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
                    >
                        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-12 h-1.5 bg-text-secondary/20 rounded-full z-30" />
                        ...
                        <div className="overflow-y-auto no-scrollbar pb-32">
                            ...
                            <div className="px-8 -mt-6 relative z-10">
                                <h1 className="font-display font-bold text-[28px] md:text-[32px] text-text-primary leading-[1.1] mb-2">
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <div className="flex items-center gap-1.5 bg-text-primary/5 px-2.5 py-1 rounded-full border border-border">
                                        <Star size={14} className="text-accent-gold" fill="currentColor" />
                                        <span className="text-[13px] font-semibold text-text-primary">{dish.rating || 4.8}</span>
                                    </div>
                                </div>
                                ...
                                <div className="mb-6">
                                    <h3 className="text-text-primary font-bold text-[18px] mb-3 italic font-display">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-surface via-surface to-transparent pt-10">
                            {quantity > 0 ? (
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center bg-text-primary/5 border border-border rounded-[22px] p-1 h-[56px] flex-1">
                                        <button 
                                            onClick={() => onUpdateQuantity(-1)}
                                            className="w-12 h-full flex items-center justify-center text-text-primary/50 hover:text-text-primary transition-colors"
                                        >
